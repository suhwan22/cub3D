...
/*   Updated: 2023/05/08 22:01:33 by jeseo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_fuel_img(t_info *info)
{
	...
	if (init_xpm_img(info, &xpm, "asset/fuel.xpm") == ERROR)
		return (ERROR);
	if (init_new_img(info, &info->screen.fuel, fuel_size, fuel_size) == ERROR)
		return (ERROR);
	bar_w = (SCREEN_W - 10 * (int)(64.0 / fuel_size)) - \
			(SCREEN_W / 10 * 8 + fuel_size);
	bar_h = 43 * (int)(64.0 / fuel_size);
	if (init_new_img(info, &info->screen.fuel_bar, bar_w, bar_h) == ERROR)
		return (ERROR);
	draw_fuel(info, &xpm);
	draw_fuel_bar(info);
